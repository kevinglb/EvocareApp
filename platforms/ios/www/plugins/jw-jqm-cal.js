(function($) {
   $.jqmCalendar = function(element, options) {
      ...
      function init() {
         ...
         $previous = $("<a href='#' data-role='button'  class='previous-btn'><i class='icon icon-angle-right icon-reverse icon-1x '</i></a>").click(function(event) {
            refresh(new Date(plugin.settings.date.getFullYear(), plugin.settings.date.getMonth() - 1, plugin.settings.date.getDate()));
         }).appendTo($th);
         
         $header = $("<span class='md-font'><span/>").appendTo($th);
         
         $previous = $("<a href='#' data-role='button' class='next-btn'><i class='icon icon-angle-right icon-1x'</i></a>").click(function(event) {
            refresh(new Date(plugin.settings.date.getFullYear(), plugin.settings.date.getMonth() + 1, plugin.settings.date.getDate()));
         }).appendTo($th);
      ...
      //                 begin = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0, 0),
      //                 end = new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1, 0, 0, 0, 0);
      //                 event = plugin.settings.events[j]; j++ ){
      //             if(event[plugin.settings.end] >= begin && event[plugin.settings.begin] <= begin ){
      //               $span.append('&bull;'); 
      //             }
      //           }
      //           // for(var j =0;j<plugin.settings.events.length;j++)
      //           // {
      ...
      function cellClickHandler(event) {
         var $this = $(this),
            date = $this.data('date');
         $tbody.find("td.active").removeClass("active");
         ...
         for ( var   i = 0, event; event = plugin.settings.events[i]; i++ ) {
            if ( event[plugin.settings.end] >= begin && event[plugin.settings.begin] <= begin ) {
               ...
               //$("<li>" + date +" "+months[month]+ " "+( ( timeString != "00:00-00:00" ) ? timeString : "" ) + " " + title + '<br>'+type+"</li>").appendTo($listview);
                  $("<div class='item'><div class='row col-xs-5 md-font'>"+ time+"</div><div class='row col-xs-7 md-font'>"+title+"</div><div class='row col-xs-12' >"+type+"</div></div>").appendTo($slider_inner);
