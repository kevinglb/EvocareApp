(function($) {
   $.jqmCalendar = function(element, options) {
      ...
      function init() {
         ...
         $previous = $("<a href='#' data-role='button'  class='previous-btn'><i class='fa fa-angle-left'</i></a>").click(function(event) {
         ...
         $previous = $("<a href='#' data-role='button' class='next-btn'><i class='fa fa-angle-right'</i></a>").click(function(event) {
            refresh(new Date(plugin.settings.date.getFullYear(), plugin.settings.date.getMonth() + 1, plugin.settings.date.getDate()));
         }).appendTo($th);
